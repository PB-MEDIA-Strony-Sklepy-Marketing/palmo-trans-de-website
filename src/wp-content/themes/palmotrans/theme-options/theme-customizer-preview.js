...
( function( $ ) {
	...
	var $style = $( '#truck-customizer-inline-css' ),
		api    = wp.customize;

	// Prepare inline styles in the preview window
	if ( $style.length === 0 ) {
		var style_container = '<' + 'style type="text/css" id="truck-customizer-inline-css" />',
			$theme_css      = $( '#truck-custom-inline-css' );
		if ($theme_css.length === 0) {
			$theme_css = $( '#truck-custom-css' );
		}
		if ($theme_css.length === 0) {
		...
		$style = $( '#truck-customizer-inline-css' );
	}
	...
	api.bind( 'preview-ready', function() {
		...
		api.preview.bind( 'refresh-other-controls', function( obj ) {
			...
			if (id.indexOf( 'body_style' ) === 0) {
				...
				$( '.sc_layouts_logo .logo_text' ).html( truck_prepare_macros( val ) );

			} else if (id == 'blogdescription') {
				$( '.sc_layouts_logo .logo_slogan' ).html( truck_prepare_macros( val ) );

			} else if (id == 'copyright') {
				$( '.copyright_text' ).html( truck_prepare_macros( val ) );
				...
				if (id == 'front_page_bg_image') {
					...
					if (color_type == 'none') {
						...
						color = typeof truck_color_schemes[scheme] != 'undefined' ? truck_color_schemes[scheme]['colors']['bg_color'] : '';
					}
					$( '.front_page_section_' + section + '_inner' ).css( 'background-color', color == '' ? 'transparent' : ( mask == 1 ? color : truck_hex2rgba( color, mask ) ) );
					...
					.height( 'auto' )
					.toggleClass( 'truck-full-height sc_layouts_flex sc_layouts_columns_middle', val );
					$( window ).trigger( 'resize' );
				}
			}
		} );
		...
		function add_color_scheme_helpers() {
			$( '[class*="scheme_"]:not(html)' ).each( function() {
				var $self = $( this ),
					...
					scheme = truck_get_class_by_prefix( classes, 'scheme_' ).replace( 'scheme_', '' );
				$self.append( '<div class="truck_customizer_scheme_helper">'
									+ '<span class="truck_customizer_scheme_helper_name">'
										+ tag
										+ ( id
											? '#' + id
											: ( classes
		...
		function remove_color_scheme_helpers() {
			$( '.truck_customizer_scheme_helper' ).remove();
		}
