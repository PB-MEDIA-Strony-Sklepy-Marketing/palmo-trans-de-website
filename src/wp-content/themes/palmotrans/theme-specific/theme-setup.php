...
if ( ! defined( 'TRUCK_THEME_FREE' ) ) {
	define( 'TRUCK_THEME_FREE', false );
}
if ( ! defined( 'TRUCK_THEME_FREE_WP' ) ) {
	define( 'TRUCK_THEME_FREE_WP', false );
}

// If this theme support external updates
if ( ! defined( 'TRUCK_THEME_ALLOW_UPDATE' ) ) {
	define( 'TRUCK_THEME_ALLOW_UPDATE', true );
}

// If this theme uses skins
if ( ! defined( 'TRUCK_ALLOW_SKINS' ) ) {
	define( 'TRUCK_ALLOW_SKINS', true );
}
if ( ! defined( 'TRUCK_DEFAULT_SKIN' ) ) {
	define( 'TRUCK_DEFAULT_SKIN', 'default' );
}
if ( ! defined( 'TRUCK_REMEMBER_SKIN' ) ) {
	define( 'TRUCK_REMEMBER_SKIN', false );
}
...
if ( ! function_exists( 'truck_get_theme_pro_key' ) ) {
	/**
	 ...
	 */
	function truck_get_theme_pro_key() {
		$theme_pro_key = get_option( sprintf( 'purchase_code_src_%s', get_template() ) );
		if ( empty( $theme_pro_key ) ) {
			$theme_pro_key = truck_storage_get( 'theme_pro_key' );
		}
		return $theme_pro_key;
	}
}

if ( ! function_exists( 'truck_trx_updater_get_theme_info' ) ) {
	add_filter( 'trx_updater_filter_get_theme_info', 'truck_trx_updater_get_theme_info' );
	/**
	 * Add an information about the theme engine to the array with a theme info.
	 *
	 * Hooks: add_filter( 'trx_updater_filter_get_theme_info', 'truck_trx_updater_get_theme_info' );
	 *
	 * @param array $info  An array with a theme information.
	 *
	 * @return array       A theme information with an engine info.
	 */
	function truck_trx_updater_get_theme_info( $info ) {
		$engine = truck_storage_get( 'engine' );
...
if ( ! function_exists( 'truck_original_theme_slug' ) ) {
	add_filter( 'truck_filter_original_theme_slug', 'truck_original_theme_slug' );
	/**
	 ...
	 *
	 * Hooks: add_filter( 'truck_filter_original_theme_slug', 'truck_original_theme_slug' );
	 *
	 * @param string $theme_slug  Current slug of the theme.
	 *
	 * @return string             Original slug of the theme.
	 */
	function truck_original_theme_slug( $theme_slug = '' ) {
		return 'truck';
	}
}
...
if ( ! function_exists( 'truck_importer_set_options' ) ) {
	add_filter( 'trx_addons_filter_importer_options', 'truck_importer_set_options', 9 );
	/**
	 * Set a theme-specific options for the import of the demo data via plugin ThemeREX Addons.
	 *
	 * Hooks: add_filter( 'trx_addons_filter_importer_options', 'truck_importer_set_options', 9 );
	 *
	 * @param array $options  Options for the import of the demo data.
	 *
	 * @return array          Processed options with a theme-specific values.
	 */
	function truck_importer_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			...
			// Prepare demo data
			$options['demo_url'] = esc_url( truck_get_protocol() . ':' . truck_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
			$options['required_plugins'] = array_keys( truck_storage_get( 'required_plugins' ) );
...
if ( ! function_exists( 'truck_ocdi_set_options' ) ) {
	add_filter( 'trx_addons_filter_ocdi_options', 'truck_ocdi_set_options', 9 );
	/**
	 ...
	 * @return array          Processed options with a theme-specific values.
	 */
	function truck_ocdi_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			// Prepare demo data
			$options['demo_url'] = esc_url( truck_get_protocol() . ':' . truck_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
...
if ( ! function_exists( 'truck_customizer_theme_setup1' ) ) {
	add_action( 'after_setup_theme', 'truck_customizer_theme_setup1', 1 );
	/**
	 * Create an internal setting for the theme.
	 * Attention! This settings ONLY FOR PROGRAMMERS, NOT FOR CUSTOMER
	 *
	 * Hooks: add_action( 'after_setup_theme', 'truck_customizer_theme_setup1', 1 );
	 *
	 ...
	 */
	function truck_customizer_theme_setup1() {
		truck_storage_set(
			'settings', array(
				...
				'allow_fullscreen'          => false,                   // Allow cases 'fullscreen' and 'fullwide' (if a next settings is true) for the body style in the Theme Options
																		...
																		// (can be removed if filter 'truck_filter_allow_fullscreen' return false)
