...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			truck_create_observer( 'truck-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				truck_gutenberg_editor_init();
			} );
		} else {
		 	truck_gutenberg_editor_init();
		}
	...
	function truck_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor.$frame.length ) {
					if ( editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'truck_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'truck_inited' ) ) {
	...
	// Init on page load
	function truck_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = truck_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + TRUCK_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + TRUCK_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in TRUCK_STORAGE['override_classes'] ) {
				$obj.addClass( TRUCK_STORAGE['override_classes'][i].replace( '%s', TRUCK_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( truck_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && TRUCK_STORAGE['sidebar_position'] != 'hide' ) {
				if ( truck_apply_filters( 'truck_filter_wrap_gutenberg_editor_with_sidebar', false ) ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'truck_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			if ( truck_apply_filters( 'truck_filter_add_scheme_to_widgets_editor', true ) ) {
				...
				truck_remove_observer( 'truck-check-editor-styles-wrapper' );
				truck_create_observer( 'truck-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'truck_inited' );
	}
} );
