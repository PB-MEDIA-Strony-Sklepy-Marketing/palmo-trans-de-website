...
jQuery( document ).ready(
	function() {
		...
		truck_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="truck_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.truck_post_editor' );
						if ( link.length > 0 ) {
							truck_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.truck_post_editor', function(e) {
						truck_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('truck_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function truck_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
