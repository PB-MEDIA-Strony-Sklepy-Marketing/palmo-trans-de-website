...
if ( ! function_exists( 'truck_storage_get' ) ) {
	/**
	 ...
	 */
	function truck_storage_get( $var_name, $default = '' ) {
		global $TRUCK_STORAGE;
		return isset( $TRUCK_STORAGE[ $var_name ] ) ? $TRUCK_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'truck_storage_set' ) ) {
	/**
	 ...
	 */
	function truck_storage_set( $var_name, $value ) {
		global $TRUCK_STORAGE;
		$TRUCK_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'truck_storage_empty' ) ) {
	/**
	 ...
	 */
	function truck_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $TRUCK_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $TRUCK_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $TRUCK_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $TRUCK_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'truck_storage_isset' ) ) {
	/**
	 ...
	 */
	function truck_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $TRUCK_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $TRUCK_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $TRUCK_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $TRUCK_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'truck_storage_unset' ) ) {
	/**
	 ...
	 */
	function truck_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $TRUCK_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			if ( isset( $TRUCK_STORAGE[ $var_name ][ $key ][ $key2 ] ) ) {
				unset( $TRUCK_STORAGE[ $var_name ][ $key ][ $key2 ] );
			}
		} elseif ( '' !== $key ) {
			if ( isset( $TRUCK_STORAGE[ $var_name ][ $key ] ) ) {
				unset( $TRUCK_STORAGE[ $var_name ][ $key ] );
			}
		} else {
			if ( isset( $TRUCK_STORAGE[ $var_name ] ) ) {
				unset( $TRUCK_STORAGE[ $var_name ] );
			}
		}
	}
}

if ( ! function_exists( 'truck_storage_inc' ) ) {
	/**
	 ...
	 */
	function truck_storage_inc( $var_name, $value = 1 ) {
		global $TRUCK_STORAGE;
		if ( empty( $TRUCK_STORAGE[ $var_name ] ) ) {
			$TRUCK_STORAGE[ $var_name ] = 0;
		}
		$TRUCK_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'truck_storage_concat' ) ) {
	/**
	 ...
	 */
	function truck_storage_concat( $var_name, $value ) {
		global $TRUCK_STORAGE;
		if ( empty( $TRUCK_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'truck_storage_get_array' ) ) {
	/**
	 ...
	 */
	function truck_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $TRUCK_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $TRUCK_STORAGE[ $var_name ][ $key ] ) ? $TRUCK_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $TRUCK_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $TRUCK_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'truck_storage_set_array' ) ) {
	/**
	 ...
	 */
	function truck_storage_set_array( $var_name, $key, $value ) {
		global $TRUCK_STORAGE;
		if ( ! isset( $TRUCK_STORAGE[ $var_name ] ) ) {
			$TRUCK_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $TRUCK_STORAGE[ $var_name ] ) ) {
			if ( '' === $key ) {
				$TRUCK_STORAGE[ $var_name ][] = $value;
			} else {
...
if ( ! function_exists( 'truck_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function truck_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $TRUCK_STORAGE;
		if ( ! isset( $TRUCK_STORAGE[ $var_name ] ) ) {
			$TRUCK_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $TRUCK_STORAGE[ $var_name ][ $key ] ) ) {
			$TRUCK_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( is_array( $TRUCK_STORAGE[ $var_name ][ $key ] ) ) {
			if ( '' === $key2 ) {
				$TRUCK_STORAGE[ $var_name ][ $key ][] = $value;
			} else {
				$TRUCK_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
			}
		}
	}
}

if ( ! function_exists( 'truck_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function truck_storage_merge_array( $var_name, $key, $value ) {
		global $TRUCK_STORAGE;
		if ( ! isset( $TRUCK_STORAGE[ $var_name ] ) ) {
			$TRUCK_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$TRUCK_STORAGE[ $var_name ] = array_merge( $TRUCK_STORAGE[ $var_name ], $value );
		} else {
			$TRUCK_STORAGE[ $var_name ][ $key ] = array_merge( $TRUCK_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'truck_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function truck_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $TRUCK_STORAGE;
		if ( ! isset( $TRUCK_STORAGE[ $var_name ] ) ) {
			$TRUCK_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			truck_array_insert_after( $TRUCK_STORAGE[ $var_name ], $after, $key );
		} else {
			truck_array_insert_after( $TRUCK_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'truck_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function truck_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $TRUCK_STORAGE;
		if ( ! isset( $TRUCK_STORAGE[ $var_name ] ) ) {
			$TRUCK_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			truck_array_insert_before( $TRUCK_STORAGE[ $var_name ], $before, $key );
		} else {
			truck_array_insert_before( $TRUCK_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'truck_storage_push_array' ) ) {
	/**
	 ...
	 */
	function truck_storage_push_array( $var_name, $key, $value ) {
		global $TRUCK_STORAGE;
		if ( ! isset( $TRUCK_STORAGE[ $var_name ] ) ) {
			$TRUCK_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $TRUCK_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $TRUCK_STORAGE[ $var_name ][ $key ] ) ) {
				$TRUCK_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $TRUCK_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'truck_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function truck_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $TRUCK_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $TRUCK_STORAGE[ $var_name ] ) && is_array( $TRUCK_STORAGE[ $var_name ] ) && count( $TRUCK_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $TRUCK_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $TRUCK_STORAGE[ $var_name ][ $key ] ) && is_array( $TRUCK_STORAGE[ $var_name ][ $key ] ) && count( $TRUCK_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $TRUCK_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'truck_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function truck_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $TRUCK_STORAGE;
		if ( ! isset( $TRUCK_STORAGE[ $var_name ] ) ) {
			$TRUCK_STORAGE[ $var_name ] = array();
		}
		if ( empty( $TRUCK_STORAGE[ $var_name ][ $key ] ) ) {
			$TRUCK_STORAGE[ $var_name ][ $key ] = 0;
		}
		$TRUCK_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'truck_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function truck_storage_concat_array( $var_name, $key, $value ) {
		global $TRUCK_STORAGE;
		if ( ! isset( $TRUCK_STORAGE[ $var_name ] ) ) {
			$TRUCK_STORAGE[ $var_name ] = array();
		}
		if ( empty( $TRUCK_STORAGE[ $var_name ][ $key ] ) ) {
			$TRUCK_STORAGE[ $var_name ][ $key ] = '';
		}
		$TRUCK_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'truck_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function truck_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $TRUCK_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $TRUCK_STORAGE[ $var_name ] ) ? $TRUCK_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $TRUCK_STORAGE[ $var_name ] ) ? $TRUCK_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'truck_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function truck_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $TRUCK_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $TRUCK_STORAGE[ $var_name ]->$prop ) ? $TRUCK_STORAGE[ $var_name ]->$prop : $default;
	}
}
