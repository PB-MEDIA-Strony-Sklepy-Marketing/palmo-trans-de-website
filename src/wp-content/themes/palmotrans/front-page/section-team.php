<div class="front_page_section front_page_section_team<?php
	$truck_scheme = truck_get_theme_option( 'front_page_team_scheme' );
	if ( ! empty( $truck_scheme ) && ! truck_is_inherit( $truck_scheme ) ) {
		echo ' scheme_' . esc_attr( $truck_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( truck_get_theme_option( 'front_page_team_paddings' ) );
	if ( truck_get_theme_option( 'front_page_team_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$truck_css      = '';
		$truck_bg_image = truck_get_theme_option( 'front_page_team_bg_image' );
...
<?php
	// Add anchor
	$truck_anchor_icon = truck_get_theme_option( 'front_page_team_anchor_icon' );
	$truck_anchor_text = truck_get_theme_option( 'front_page_team_anchor_text' );
...
?>
	...
	<?php
	if ( truck_get_theme_option( 'front_page_team_fullheight' ) ) {
	...
	?>
			...
			<?php
			$truck_css      = '';
			$truck_bg_mask  = truck_get_theme_option( 'front_page_team_bg_mask' );
			$truck_bg_color_type = truck_get_theme_option( 'front_page_team_bg_color_type' );
			if ( 'custom' == $truck_bg_color_type ) {
				$truck_bg_color = truck_get_theme_option( 'front_page_team_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_team_content_wrap content_wrap">
			<?php
			// Caption
			$truck_caption = truck_get_theme_option( 'front_page_team_caption' );
			if ( ! empty( $truck_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_team_caption front_page_block_<?php echo ! empty( $truck_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $truck_caption, 'truck_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$truck_description = truck_get_theme_option( 'front_page_team_description' );
			if ( ! empty( $truck_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_team_description front_page_block_<?php echo ! empty( $truck_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $truck_description ), 'truck_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_team_output">
				...
				if ( is_active_sidebar( 'front_page_team_widgets' ) ) {
					...
					if ( ! truck_exists_trx_addons() ) {
						...
						truck_customizer_need_widgets_message( 'front_page_team_caption', 'ThemeREX Addons - Team' );
					}
				}
				?>
			</div>
